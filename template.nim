when not declared(LIBRARY_TEMPLATE):
  ...
  import
    math, lenientops, bitops, strutils, nre, parseutils, sequtils, algorithm, sets,
    tables, deques, heapqueue, macros
  import sugar except dump

  template inf(T: typedesc[int]): int =
    10 ^ 18

  template inf(T: typedesc[float]): float =
    1e18
  ...
  proc `+`*[T](a, b: (T, T)): (T, T) {.inline.} =
    (a[0] + b[0], a[1] + b[1])

  proc `+`*[T](a, b: (T, T, T)): (T, T, T) {.inline.} =
  ...
  proc `*`*[T](a: (T, T), k: T): (T, T) {.inline.} =
    (a[0] * k, a[1] * k)

  proc `*`*[T](a: (T, T, T), k: T): (T, T, T) {.inline.} =
    (a[0] * k, a[1] * k, a[2] * k)

  proc `div`*[T](a: (T, T), k: T): (T, T) {.inline.} =
    (a[0] div k, a[1] div k)

  proc `div`*[T](a: (T, T, T), k: T): (T, T, T) {.inline.} =
    (a[0] div k, a[1] div k, a[2] div k)

  proc `mod`*[T](a: (T, T), k: T): (T, T) {.inline.} =
    (a[0] mod k, a[1] mod k)

  proc `mod`*[T](a: (T, T, T), k: T): (T, T, T) {.inline.} =
    (a[0] mod k, a[1] mod k, a[2] mod k)
  ...
  template makeSeq[T](len: int, init: T): auto =
    newSeqWith(len, init)

  template makeSeq(len: int, init: typedesc): auto =
    newSeq[init](len)
  ...
  template calcIdx*(len: int, i: int): int =
    i

  template calcIdx*(len: int, i: BackwardsIndex): int =
    len - int(i)
  ...
  macro forElse*(loopExpr: untyped, body: untyped): untyped =
    ...
    for node in stmtList:
      if not foundElse and node.kind == nnkCall and node[0].kind == nnkIdent and
          node[0].strVal == "Else":
        foundElse = true
  ...
  macro query*(countExpr: typed, body: untyped): untyped =
    ...
    for node in body:
      if node.kind in {nnkCall, nnkCommand} and node[0].kind == nnkIdent and
          node[0].strVal == "op":
        let queryId = node[1]
